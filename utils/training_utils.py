...
INSTRUCTION = "You are the brain of an autonomous vehicle (ego vehicle) and please summarize the scene based on the input scenario."
ACTION_Q = [
...
def get_train_val_data(
    ...
    if processed_train_data_path is None:
        if vqa:
    ...
    if val_set_size > 0:
        ...
        if processed_train_data_path is None:
            ...
            train_data = train_data.map(
                ...
                remove_columns=[],
                num_proc=8,
            )# Here we use the tokenizer to tokenize the input and output prompts, and then we add the labels to the tokenized prompts
            
            # devide the train_data into chunks with 80 samples
            # subsets_list = []
            # for i in range(0, len(train_data), 10000):
            #     subsets_list.append(train_data.select(range(i, min(i+10000, len(train_data)))))

            # for i in range(len(subsets_list)):
            #     subsets_list[i] = subsets_list[i].map(
            #         partial(generate_and_tokenize_prompt, tokenizer, user_input_ids=True),
            #         remove_columns=[],
            #         num_proc=8,
            #     )
            # train_data = concatenate_datasets(subsets_list)

            # save the train_data to the disk
            #train_data.save_to_disk('processed_train_data')
        else:
            train_data = load_from_disk(processed_train_data_path)
...
def generate_and_tokenize_prompt(tokenizer, data_point, user_input_ids=False):
    ...
    # Note that here the data_point['input'] is empty
    tokenized_full_prompt = tokenize(tokenizer, full_prompt)# Here we tokenize the full prompt, convert to token ids

    user_prompt = generate_prompt({**data_point, "output": ""}) # Note that here we replace the output with an empty string
    ...
    tokenized_full_prompt["lane_geom_descriptors"] = data_point["lane_geom_descriptors"]
    tokenized_full_prompt["lane_topo_descriptors"] = data_point["lane_topo_descriptors"]
    tokenized_full_prompt["agents_descriptors"] = data_point[
        "agents_descriptors"
    ]
    tokenized_full_prompt["ego_vehicle_descriptor"] = data_point[
        "ego_vehicle_descriptor"
    ]
    #tokenized_full_prompt["bev_descriptors"] = data_point["bev_descriptors"]
    tokenized_full_prompt["plan_query"] = data_point["plan_query"]
    tokenized_full_prompt["ego_command_descriptors"] = data_point[
        "ego_command_descriptors"
    ]
    tokenized_full_prompt["instruction"] =  tokenize(tokenizer, data_point["instruction"], padding="max_length", cutoff_len=150, add_eos_token=False)['input_ids']
...
def tokenize(tokenizer, prompt, cutoff_len=512, padding=False, add_eos_token=True):
    ...
    result["labels"] = result["input_ids"].copy() # Why the labels are the same as the input ids?
...
def _load_vqa_pickle_dataset(
    ...
    for d in data:
        ...
        if "vqa" in dataset_items:
            ...
            for json_dict in parser(d["response_content"]): # Extract  QA from the response content
...
def _load_vector_pkl_dataset(data_path, augment_times):
    ...
    for d in data:
        data_dict["frame_num"].append(d["frame_num"])
        data_dict["input"].append("")
        #data_dict["output"].append(make_observation_prompt(d["observation"])) # This step converts the vector observation to a text scene description
        data_dict["output"].append(d["input_prompt"])
        # Append the descriptors to the data_dict
        _append_descriptors(data_dict, d["observation"])
